...
function activate(context) {
    ...
    const contextMenu = vscode.commands.registerCommand('code2md.generateMarkdownContext', async (...args) => {
        ...
        try {
            ...
            if (args.length === 2 && Array.isArray(args[1]) &&
                args[1].every((item) => item instanceof Object && item.fsPath)) {
            ...
            // Handle the case where an array is the first argument
                ...
                args[0].every((item) => item instanceof Object && item.fsPath)) {
                selectedFiles = args[0];
                logToFile('[code2md] Files from first argument array');
            }
            // Handle multiple URI arguments directly
            else if (args.length > 0 && args.every((arg) => arg instanceof Object && arg.fsPath)) {
                selectedFiles = args;
            ...
            if (selectedFiles.length === 0) {
                for (const arg of args) {
                    if (arg && Array.isArray(arg)) {
                        const uris = arg.filter((item) => item && typeof item === 'object' && item.fsPath);
                        if (uris.length > 0) {
                            selectedFiles = uris;
